...
public class EmptyBiomeBottle extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        if (!pLevel.getBiome(pPlayer.blockPosition()).is(DataGenerators.BiomeTagGenerator.BOTTLE_BLACKLIST)) {
    ...
    public ItemStack finishUsingItem(ItemStack pStack, Level pLevel, LivingEntity pLivingEntity) {
        if (pLivingEntity instanceof Player player) {
            if (!pLevel.getBiome(pLivingEntity.blockPosition()).is(DataGenerators.BiomeTagGenerator.BOTTLE_BLACKLIST)) {

...
public class AreaBreakItemUsage {
    ...
    public static void onAreaBreakItemUsage(BlockEvent.BreakEvent event) {
        ...
        if (mainHandItem.getItem() instanceof DiggerItem tool && mainHandItem.getItem() instanceof IAreaBreakItem areaItem && player instanceof ServerPlayer serverPlayer) {
            ...
            for (BlockPos pos : areaItem.getBlocksToDestroy(initial, serverPlayer)) {
                if (pos.equals(initial) || !tool.isCorrectToolForDrops(mainHandItem, event.getLevel().getBlockState(pos))) {
                    continue;
                }
